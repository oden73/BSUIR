...
class LogicalExpression:
    def __init__(self, expression):
        ...
        self.__indexes_dict, self.__values_dict = self.form_indexes_and_values_dict()
    ...
    def evaluate(self, syntax_subtree, expression, values_list):
        ...
        if left_child and right_child:
            ...
            return values_list[self.__indexes_dict[syntax_subtree.getRootVal()[0]]]
    ...
    def truth_table(self):
        ...
        number = [False for _ in range(0, bits)]
        print_string = ''
        for i in operands:
            print_string += 'I ' + i + ' '
        for i in subtrees:
    ...
    def form_indexes_and_values_dict(self) -> (dict, dict):
        chars: list[str] = []
        for i in self.__expression:
            if 'a' <= i <= 'z' and i not in chars:
                chars.append(i)
        chars.sort()
        indexes_dict = {}
        values_dict = {}
        for i in range(len(chars)):
            indexes_dict[chars[i]] = i
            values_dict[i] = chars[i]
        return indexes_dict, values_dict
