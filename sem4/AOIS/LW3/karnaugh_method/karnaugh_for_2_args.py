...
class KarnaughFor2Arguments(Karnaugh):
    ...
    def form_table(self) -> list[list[int]]:
        for i in self.__left_column:
            for j in range(len(self.__top_row)):
                self.__index_order.append(self.decimal_view(i + self.__top_row[j]))
        table: list[list[int]] = []
        index: int = 0
    ...
    def form_final_normal_form_arguments(self) -> list[list[str]]:
        ...
        for i in range(len(fields)):
            initial_values: list[str] = [self.__left_column[fields[i][0][0]], self.__top_row[fields[i][0][1]]]
            changes: list[bool] = [False, False]
            for j in fields[i]:
                if self.__top_row[j[1]] != initial_values[1]:
                    changes[1] = True
    ...
    def print_table(self):
        row_print = ' ' + self.__arguments[0] + '\\' + self.__arguments[1]
        for i in self.__top_row:
            row_print += '   ' + i
        print(row_print)
